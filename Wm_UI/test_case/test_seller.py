...
class Seller(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(self):
        url = readConfig.getHTTP('scheme') + r'://' +readConfig.getHTTP('host')
        self.base_url = url

        self.username = readConfig.getUserInfo('userName')
        self.password = readConfig.getUserInfo('passWord')
    ...
    def test_aOrderDetail(self):
        ...
        try :
            ...
            # 查看订单详情
            time.sleep(1)
            detail_tag = driver.find_element_by_xpath(".//*[@id='cly_order_list']/li/form/table/thead/tr/th[1]/span[3]/span/a").text
    ...
    def atest_cReleases(self):
        ...
        try:
            ...
            releaseFile = r'D:\WorkSpace\Python\Wm_UI\caseData' + '\itemRelease.xlsx'
            #print releaseFile_Path
            time.sleep(2)
    ...
    def atest_eItemsUpdate(self):
        ...
        if releasesStatus == 1:
            try:
                ...
                updataFile = r'D:\WorkSpace\Python\Wm_UI\caseData' + '\itemUpdate.xlsx'
                time.sleep(2)
                driver.find_element_by_css_selector(".webuploader-pick").click()
                time.sleep(2)
                # 将路径输入
