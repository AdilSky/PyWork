#-*-coding:utf-8-*-
# Time:2017/10/27 22:40
# Author:YangYangJun


import smtplib
import time
from email.header import Header
from email.mime.multipart import MIMEMultipart
from email.mime.text import MIMEText

from selenium import webdriver

import Wm_UIDemo.readConfig as readConfig
from Log import MyLog
readConfig = readConfig.ReadConfig()

class Email(object):
    '创建一个邮件类'

    def __init__(self):
        '初始化属性'
        self.server = readConfig.getMail('Smtp_Server')
        self.sender = readConfig.getMail('Smtp_Sender')
        self.password = readConfig.getMail('Password')
        self.LReceiver = readConfig.getMail('OnLine_Receiver')
        self.PReceiver = readConfig.getMail('Pre_Receiver')
        self.TReceiver = readConfig.getMail('Test_Receiver')
        self.Msg_Title = readConfig.getMail('Msg_Title')
        self.Content_Type = readConfig.getMail('Content_Type')
        self.Content_Disposition = readConfig.getMail('Content_Disposition')
        self.resultPath = readConfig.getMail('resultPath')

        # 实例化MyLog
        # 并传出 logger() 的参数，
        self.log = MyLog.get_log('Email')
        self.logger = self.log.logger


        # 实例化msg
        self.msg = MIMEMultipart()

    def get_Result(self,reportFile):
        '获取测试结果'
        self.driver = webdriver.Firefox()
        self.driver.maximize_window()
        ##得到测试报告路径
        self.result_url = "file://%s" % reportFile
        self.driver.get(self.result_url)
        time.sleep(3)
        resultPath = self.resultPath

        self.result = self.driver.find_element_by_xpath(resultPath).text
        self.result = self.result.split(':')
        self.driver.quit()
        self.setHeader(self.result[-1])

    def setHeader(self,result):
        '设置邮件主题'
        now = time.strftime("%Y-%m-%d-%H_%M_%S")
        # 设置邮件主题 必须项
        self.msg['subject'] = Header('[执行结果：' + result + ']'+ self.Msg_Title + now, 'utf-8')

    def setContent(self,reportFile):
        '设置邮件正文'
        f = open(reportFile, 'rb')
        # 读取测试报告正文
        self.mail_body = f.read()
        f.close()
        self.contentText = MIMEText(self.mail_body, 'html', 'UTF-8')
        self.msg.attach(self.contentText)
        self.setAccessory(self.mail_body)

    def setAccessory(self,mail_body):
        '设置附件'
        self.accessory = MIMEText(mail_body, 'html', 'utf-8')
        self.accessory['Content-Type'] = self.Content_Type
        self.accessory["Content-Disposition"] = self.Content_Disposition
        self.msg.attach(self.accessory)

        # 定义发送邮件函数，只需要传报告的绝对路径即可
    def sendEMail(self,reportFile):
        '发送邮件'
        self.get_Result(reportFile)
        self.setContent(reportFile)
        try:
            self.smtp = smtplib.SMTP(self.server,25)
            self.smtp.login(self.sender,self.password)
            self.receiver = []
            self.receiver.append(self.TReceiver)
            # 定义发件人，如果不写，发件人为空
            self.msg['From'] = self.sender
            # 定义收件人，如果不写，收件人为空
            self.msg['To'] = ",".join(self.receiver)
            self.smtp.sendmail(self.sender, self.receiver, self.msg.as_string())
            self.smtp.quit()
            self.logger.info("The test report has send to developer by email.")
            return True
        except smtplib.SMTPException as e:
            self.logger.info(str(e))
            print(str(e))
            return False




