...
class default(unittest.TestCase):
    ...
    def undirectuser_login(self):
        '''非直连用户登录'''
        self.base_url = readConfig.getHTTP('base_url')

        self.username = readConfig.getUserInfo('undirectName')
        self.password = readConfig.getUserInfo('undirectPassword')
    ...
    def directuser_login(self):
        ...
        self.floatWindow = readConfig.getCaseSet('FloatWindow')
        self.driver = webdriver.Firefox()
        time.sleep(1)
        self.driver.maximize_window()
        self.driver.get("%s/user/tologin"%self.base_url)
    ...
    def test_directuser_clean_cart(self):
        ...
        try:
            ...
            if int(cartNum) != 0:
                ...
                time.sleep(2)
                self.handleList = self.driver.window_handles
                self.driver.switch_to.window(self.handleList[-1])
                time.sleep(2)
                # 点击全部删除  
                # self.driver.find_element_by_class_name('del-shopping').find_elements_by_tag_name('button')[1].click()
